...
if (tv && stage) {
  ...
  function renderGlance() {
    stage.innerHTML =
      ...
      `</div><div class="col-label">This location · all clear</div><div class="grid2">${tilesHTML(bizZones.slice(0, 4))}</div>` +
      `<div class="actions">` +
  ...
  function renderMonitor() {
    ...
    const zones = bizZones.map((z) => `<div class="zchip${z.attn ? ' attn' : ''}"><span class="zd"></span>${esc(z.n)}</div>`).join('');
    ...
    stage.innerHTML =
      ...
      `<div class="mon">` +
        ...
        `</div>` +
        `<div class="rail">` +
          `<div class="panel"><h4>Zones <span class="ct">1 attn</span></h4><div class="zones">${zones}</div></div>` +
