...
//!
//! std-only on purpose: it lives in the tauri-free flash engine, whose
//! `cargo test` in desktop-hub-core.yml runs these tests on a bare runner,
//! and `rustc --edition 2021 --test` on this one file still runs them on a
//! checkout without the apps' GTK/WebKit dependencies.
...
mod tests {
    ...
    fn ca_mode_counts_the_sealed_bytes_and_never_prints_the_pem() {
        ...
        assert_eq!(
            line,
            "TLS, CA-verified — CA certificate sealed, 59 bytes of PEM"
        );
    ...
    fn table_is_four_single_lines_and_an_unknown_byte_says_so() {
        for (i, label) in MODE_LABELS.iter().enumerate() {
            ...
            assert!(
                ...
                "row {i} describes what was sealed, never a connection"
            );
        }
