// canary-local/assets/senselab.js — The Sense Lab page driver.
//
// Renders senselab.html entirely from devices/senselab.json — the generated,
// drift-gated data (tools/gen_senselab.py parses the canary-sense firmware +
...
async function main() {
  const mount = $("#senselab");
  let data;
  try {
    const res = await fetch("devices/senselab.json");
  ...
  function renderVersionStrip(d) {
    ...
    strip.append(el("p", "fineprint hub-fresh",
      ...
      "firmware and docs/hardware/mr60bha2_radar_notes.md by tools/gen_senselab.py and " +
  ...
  function renderDeep(s, d) {
    ...
    const drop = el("div", "slab-dropped");
  ...
  function buildSandbox() {
    const pad = el("div", "wap-sandbox slab-sandbox");
    const scenarios = [
