...
# -*- coding: utf-8 -*-
"""
gen_sense.py — build canary-local/devices/sense.json from the canary-sense
firmware + the MR60BHA2 hardware notes.

The Sense Lab (`canary-local/sense.html`) stages the radar witness: the real
...
CI re-runs the generator and `git diff --exit-code`s `sense.json`, the same
...
out = {
    "$note": "GENERATED by canary-local/tools/gen_sense.py — do not edit by hand; "
...
print(f"gen_sense.py: wrote {OUT_JSON.relative_to(REPO)}  "
