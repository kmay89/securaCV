...
test("broker TLS: both flashers' receipts name the sealed mode the same way", async () => {
  ...
  // broker_receipt.rs MODE_LABELS) and this test is the only thing holding
  // the copies equal: it must fail on either one changing alone.
  const flashJs = read(join(CANARY, "assets/flash.js"));
  const coreJs = read(join(CANARY, "assets/flash-core.js"));
  const appJs = read(join(ROOT, "desktop/src/app.js"));
  const html = read(join(ROOT, "desktop/src/index.html"));
  const provRs = read(join(ROOT, "desktop/src-tauri/src/provisioning.rs"));
  const receiptRs = read(join(ROOT, "desktop/src-tauri/src/broker_receipt.rs"));
  const core = await import(pathToFileURL(join(CANARY, "assets/flash-core.js")).href);
  ...
  // Native: the log line rides under the broker branch of `flash` only, is
  // built from the summary + the std-only formatter (never a config field),
  // and the sealed line stopped saying "values not logged" now that a pin
  // is printed. The receipt carries it; a local file carries None.
  assert.match(libRs, /^mod broker_receipt;$/m, "lib.rs must register the receipt module");
  const flashFn = nativeFnBody(libRs, "flash");
  assert.ok(flashFn.includes("(passwords not logged)"), "the sealed line must say passwords, not values, are withheld");
  assert.ok(!libRs.includes("values not logged"), "'values not logged' is false once the pin is printed");
  const branch = /\n( +)if broker \{\n([\s\S]*?)\n\1\}/.exec(flashFn);
  assert.ok(branch, "lib.rs flash lost its `if broker {` branch");
  ...
  assert.match(branch[2], /emit\(&app, format!\("  broker link: \{line\}"\)\);/);
  assert.match(branch[2], /broker_tls = Some\(BrokerTlsReceipt \{ mode, ca_bytes, fingerprint, line \}\);/);
  assert.strictEqual(libRs.split("broker link:").length - 1, 1, "the broker line is emitted in exactly one place — the broker branch");
  ...
  assert.match(libRs, /pub struct FlashReceipt \{[\s\S]*?broker_tls: Option<BrokerTlsReceipt>,\n\}/, "FlashReceipt lost broker_tls");
  assert.match(libRs, /#\[derive\(Serialize\)\]\npub struct BrokerTlsReceipt \{\n\s+mode: u8,\n\s+ca_bytes: usize,\n\s+fingerprint: String,\n\s+line: String,\n\}/,
  ...
  assert.ok(!/^use /m.test(receiptRs.split("#[cfg(test)]")[0]),
    "broker_receipt.rs is std-only — the tauri-pure-modules rustc leg compiles it alone");
