...
/// `witness_api <subcommand>` on a host with no identity: no DEVICE_KEY_SEED
/// and no seed file. The daemon path creates an identity there; a subcommand
/// must not. `help` and `revoke-viewer-token` need none, and a mint refuses by
/// name. Afterwards the host still has no seed file, no database and no
/// viewer file.
#[test]
fn subcommands_never_create_a_device_identity() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let db = dir.path().join("witness.db");
    let token_path = dir.path().join("api_token");
    let viewer_path = dir.path().join("viewer_tokens.json");
    ...
    let seed_file = witness_kernel::crypto::device_key_path_for_db(&db.to_string_lossy())?;
    let run = |args: &[&str]| {
        Command::new(env!("CARGO_BIN_EXE_witness_api"))
            .args(args)
            .env("WITNESS_CONFIG", &config)
    ...
    assert!(out.status.success(), "stderr: {}", stderr(&out));
    assert!(
        stderr(&out).contains("mint-viewer-token"),
        "stderr: {}",
        stderr(&out)
    );

    let out = run(&["mint-viewer-token", "--label", "tv"]);
    assert!(!out.status.success(), "a mint with no identity must refuse");
    assert!(
        stderr(&out).contains("no device identity"),
        "stderr: {}",
        stderr(&out)
    );
    assert!(stdout(&out).is_empty(), "no receipt: {}", stdout(&out));

    let out = run(&["revoke-viewer-token", "no-such-id"]);
    assert!(!out.status.success(), "an unknown id is refused");

    assert!(!seed_file.exists(), "a subcommand minted a device identity");
    assert!(!db.exists(), "a subcommand created the database");
    assert!(
        !viewer_path.exists(),
        "a refused mint wrote the viewer file"
    );
    Ok(())
}
