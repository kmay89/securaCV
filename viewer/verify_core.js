...
async function verifyEnvelope(envelope) {
  ...
  try {
    ...
    const sealedHead = await verifyLinearChain(sealed.entries, sealedStart, domains.sealed_log_entry, pubKey, 'sealed_events', sigCheckable);
    ...
    const expHead = await verifyLinearChain(exp.entries, new Uint8Array(32), domains.export_receipt, pubKey, 'export_receipts', sigCheckable);
