...
function normalizeEnvelope(envelope) {
  ...
  for (const e of entries) {
    ...
    // A payload of `null`, a bare number, or an array all parse fine and then
    // throw on the first property read — which took down the whole timeline,
    // not just the bad row. The viewer's entire job is reading files from
    // people it does not trust, so a malformed entry is counted, never fatal.
    if (rec === null || typeof rec !== 'object' || Array.isArray(rec)) { unparsed += 1; continue; }
    let r = null;
