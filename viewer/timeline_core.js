...
function dominantFamily(cell) {
  ...
  let best = 'other', n = 0;
  for (const fam of Object.keys(cell.families)) {
    if (cell.families[fam] > n) { n = cell.families[fam]; best = fam; }
  }
  return best;
}
...
function mostCommonSize(records) {
  ...
  let best = 0, n = 0;
  for (const [size, count] of freq) { if (count > n) { n = count; best = size; } }
  return best;
}
