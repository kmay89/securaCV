...
| `tsa` | off | stable | Online RFC 3161 TSA client for `log_anchor request` and `anchor-all` (ureq + 30s timeout, 256 KiB cap). The offline query/import/`anchor-all --offline-dir` flow needs no feature; no shipped image enables this | keep (network egress stays opt-in at build time by design — see docs/timestamping.md) |
...
| `surface-busybar` | off | exp | The BUSY Bar display surface (`busybar_surface` bin): witness state on a Flipper Devices BUSY Bar's two displays over its local HTTP API — the room-facing matrix renders public-class card content only, the operator-facing rear carries the detail (docs/design/busybar_surface.md). Read-only by construction: subscribe-only broker handle, GET-only kernel access, no control can affect witnessing. Implies `alert-relay` so both lanes share one classification core and one cloud refusal | promote to stable once it has been run against real hardware — the protocol comes from the firmware's OpenAPI spec and the open items are busybar_surface.md §9 |
