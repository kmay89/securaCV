...
- `firmware/canary`'s adoption is compile-tested by the `release_ha` leg
  ...
  number. Its 3 s connect bound goes through the shared header's
  `set_connect_timeout_sec()`, so it holds on either core. Every env that
  compiles `securacv_mqtt` builds on core 2 today (`[env:full]`, the one
  core-3 canary env, leaves `FEATURE_HA_MQTT` at 0), and two
  `static_assert`s keep each connect stage under the 8 s watchdog.
