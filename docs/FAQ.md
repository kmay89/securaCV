...
| A doorway that's very hard to sneak past | **Canary Sentinel** — *not yet flashable*: the fusion core is host-tested and its network firmware compiles in CI, but nothing has run on hardware and there is no released build; use a Canary Sense or Vision today |
