...
def distill_cad_dims() -> dict:
    ...
    return {
        "comment": "GENERATED by gen_builder_manifest.py --site — do not edit. "
                   "Envelopes distilled from securaCV's fleet-figures ledger "
                   "(canary-local/devices/figures.json), which reads them from "
                   "the committed STL bounding boxes. The website's model tests "
                   "pin the AR models to these numbers.",
