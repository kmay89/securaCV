...
| A screw inside the gasket line is a drip path unless it is O-ring sealed — and a plate screw is one (its seat is a hole through the seal line from outside), so a sealed build glands every plate seat, always, under a pan head (the head's recess is limited so the ring's floor keeps a 0.4 web inside the plate) | `e_gland = e_seal` | `pl_seat_cut(gland=)`, `pl_recess(gland=)`, `pl_web()`; `oring_gland_h()` squeezes 25 %; asserted pan-head only; the `head` probe (the head fits) and `head_floor` probe (bearing material) on every sealed preset |
