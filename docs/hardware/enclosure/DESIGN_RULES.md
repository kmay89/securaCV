...
| No screw head on a show face: screws drive from a seat in the back into bosses under the face, the length, recess and boss derived together; the face keeps an unbroken skin over every tip | skin 1.0; thread engagement ≥ 3 × d; the head wholly in the floor | `bk_len()` / `bk_recess()` / `bk_boss_h()` / `bk_seat_cut()` / `bk_boss_bore()` in `canary_core_lib`; `screw_from = "back"` held by `lint_design_lang.py`; the fit gates prove each boss lands on its post |
