...
| [`bom_pipeline.md`](./bom_pipeline.md) | **How the BOMs run themselves** — design intent (the CSVs) vs fetched supply-chain facts (`pricing.json`), the nightly Digi-Key/Mouser snapshot workflow, the exception policy (out-of-stock / price-jump / lifecycle issues), and the local runbook. |
