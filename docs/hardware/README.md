...
| [`canary_vision_pro_recamera.md`](./canary_vision_pro_recamera.md) | **Canary Vision Pro (reCamera Pro) — concept** — the flagship tier: a standalone on-device NPU camera integrated with zero new firmware via the existing webhook adapter, claim mapping, what it deliberately doesn't route (VLM/LLM captions), and where the $300 price earns its keep vs. the cheaper Vision path. |
