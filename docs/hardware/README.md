...
| [`canary_car_mode.md`](./canary_car_mode.md) | **Canary Car Mode — concept** — a stock, unmodified Meshtastic node in your car's ignition-switched USB port: no firmware, no circuit, arrival on boot, departure inferred from heartbeat silence (never an unreliable OFF-transition packet — a real upstream firmware bug is why). Setup runbook + honest limits, all in one doc. |
