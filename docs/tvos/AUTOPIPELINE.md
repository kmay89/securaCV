...
| **One app, two editions** | Home (the Witness Wall) and Business (the Witness Board) are the *same binary*, choosing their edition from the kernel's profile at runtime — like the Lab's per-platform adaptive nav. Two experiences, **one** release surface: no second app to tag, sign, review, or let rot. | The Lab's "one frontend, N platforms" capability layer. |
...
| **Self-heal (on device)** | The app re-verifies the Ed25519 hash-chain and fleet manifest on every render; drift is shown **loudly and calmly**, never silently rendered as fine — the same posture as firmware's key-drift hard-fail and its "the update did not pass its health check, so the previous software was restored" message. Bonjour discovery + backoff reconnect heal network blips; a Doctor card turns any problem into one action. | Firmware A/B boot self-test + `PENDING_VERIFY` rollback; browser flasher "never get stuck." |
| **Self-heal (pipeline)** | The workflow **no-ops cleanly** until `ENABLE_TVOS_BUILD=true` and signing secrets exist — a curious "Run workflow" click or an early `tvos-v*` tag succeeds with a helpful message instead of hard-failing. | `desktop-mobile-release.yml`'s `ENABLE_IOS_BUILD` gate; the flasher's ephemeral-key fallback. |
...
| **No rot (app can't drift)** | Contract + snapshot tests: the app renders from `spec/event_contract.md` and `spec/evidence_envelope.md`; a schema change that the UI hasn't kept up with fails CI. Golden-screenshot tests keep the ambient surfaces from silently rotting. A nightly job re-runs them even with zero commits and auto-files an issue on drift. | Website `nightly-check.yml` "anti-rot suite" that auto-opens issues. |
| **No new attack surface** | The core is **byte-identical** to the kernel's verifier — the TV can't disagree with the source of truth. The app is read-only, holds no authoritative state, and never touches raw media (`spec/invariants.md`, Invariant I). | The shared-Rust-core discipline already used across desktop + firmware. |
