...
| **Appliance activity** card stays at **No activity** | Either no IR receiver is wired to GPIO 3 (D2), or no IR remotes are being used in the room. Verify with `ir_get_stats` showing zero `frames_received`. |
