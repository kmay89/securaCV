...
| **F6** | Verify | PlatformIO `platform = espressif32@6.9.0` (= core 2.0.17 / IDF 4.4) is **inconsistent** with the IDF5 APIs the code uses (`esp_task_wdt_config_t.idle_core_mask`, NimBLE 2.x). The canonical build is **arduino-cli with `esp32:esp32` latest (3.x / IDF5)** — see `.github/workflows/firmware.yml`. | No code change. **Recommendation:** either bump the PlatformIO platform to a 3.x-core source (e.g. the pioarduino fork) or document that PlatformIO is not the supported toolchain for canary-wap, to avoid a misleading/broken `pio run`. |
| **F7** | Info | No brownout/power handling in firmware. | No code change. **Recommendation:** for field units add bulk decoupling (470–1000 µF low-ESR + 0.1 µF close to the module) and ensure a ≥500 mA clean 3.3 V source; treat repeated safe-mode entry as a possible power symptom. |
