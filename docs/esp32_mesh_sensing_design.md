...
Routine probe traffic is meant to be gated by `airtime_governor::try_reserve_routine()` (introduced in #442), with urgent traffic (alerts) bypassing the cap via `force_reserve_urgent()`; the probe is routine. **Not yet landed:** the probe does not yet reserve through `airtime_governor` — the comment above `aggregate_cap_hz` in `csi_probe.h` says so in its own words: the governor only reports utilization (`csi_integration.cpp` reads `airtime_pct_x100` to throttle the HAL), probe sends are not routed through `try_reserve_routine()`, and at the 1 Mbps ESP-NOW rate the 200 Hz aggregate cap would cost ~13 % of the channel against the governor's 2 % routine budget (30 Hz is the cap that fits). Until the reservation is wired in, keep fleets small or lower the cap.
