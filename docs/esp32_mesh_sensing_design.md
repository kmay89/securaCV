...
Today CSI capture quality is governed by whatever beacons + STA traffic happen to fly by. The new design makes every paired Sensor send ESP-NOW unicasts to every paired peer at a peer-count-aware effective rate. The fixed knob is an **aggregate cap** (default 200 Hz total Tx); the per-peer rate is `min(20 Hz, aggregate_cap / max(1, peer_count))`. With 1 peer that's 20 Hz; with 8 peers it's 25 Hz capped to 20 = 20 Hz per peer for 160 Hz aggregate asked. The aggregate cap does not bound the probe's airtime; the airtime governor does. By the governor's framed estimate (792 µs a frame at 1 Mbps) 200 Hz would be ~16 % of the window, and the probe stops at 1.60 %, which is about 20 frames/s in total. So one peer at 20 Hz fits (with an Opera of one; a heartbeat to more mesh peers holds it off about 100 ms a peer, 1.6 s for a full Opera of 16), and with a filled peer table an over-budget probe arrives in bursts (host-measured: about 1.3 s of frames in every 10 s window at 8 peers, 5 s at 2 peers) unless `aggregate_cap_hz` is set from the budget. 20 Hz matches the existing CSI HAL rate-limiter ceiling so we never waste airtime on frames the receiver will drop. Each frame triggers a CSI callback on the receiver with a **known sender** on a **pinned channel**, so the 1 Hz feature window (20 frames target) becomes deterministic instead of best-effort. This alone is the single biggest accuracy and stability improvement available on this hardware.

Probe traffic is routine: urgent traffic (alerts) bypasses the governor's cap via `force_reserve_urgent()`, and the probe never does. **Landed on the WAP:** every probe send asks `csi_probe::Config::airtime_gate` first (sweep F4, #1696); a denied send is skipped, counted in `sends_denied_airtime`, and keeps its slot's cadence. The WAP installs `probe_airtime::reserve_probe_frame` (`probe_airtime.h`), which charges each frame through `airtime_governor::try_reserve_routine()` (the governor adds its ~59 B ESP-NOW framing allowance to every caller's frame), starts no frame once the governor's window reads 1.60 % (so heartbeats and presence keep the rest of the 2 % routine cap), and brings the governor up on a build without the mesh. The governor's window is summed in 100 ms buckets, so it holds every send at any rate. Host-tested (`test_csi_probe_airtime`, `test_mesh_coexistence`); the percentages are the governor's estimate, and whether it matches real air is a bench question. Today the WAP's probe table is empty, so it broadcasts at 10 Hz, ~0.8 % by that estimate.
