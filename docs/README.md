...
- [Timeline history deeper than the ring (design)](design/witness_history_bridge.md) — stage 1 built, stage 2 designed and NOT built: the canary's web timeline stops at the 32-record RAM ring although every record is on the card; the pure backward JSONL walker (any chunk size, torn-line skip, an untrusted O(page) resume hint, chain linkage — host-tested) and the loop-task SD read bridge it is for (one mailbox, a 3 s bounded wait, at most 4 KiB read per loop pass, rows badged "from card, chain-linked", never "verified")
