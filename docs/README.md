...
- [The Canary design language (design)](design/DESIGN_LANGUAGE.md) — **built and gated**: one grammar for every case and every surface that draws one — the radius family, the two-stage edge, the parting-line seam reveal, the feature vocabulary (light pipe, vent cluster, ports, mounts), the fastener philosophy, ONE canary yellow. Every rule names the code that states it and the gate that holds it (`scripts/lint_design_lang.py` fails the build on a canonical default that neither matches the house value nor explains its deviation on its own line), so the grammar cannot rot back into taste
...
- [Device manifests — one file per Canary, every fact joined](../devices/README.md) — `devices/<slug>/device.json` names, for one device, the board registry row, the build envs, the emulator twin, the fleet figure, the enclosure sets and the flasher product, and `scripts/lint_device_manifests.py` proves every one of those joins against the file that owns the fact (roadmap §4, wave 1: describe — nothing consumes the manifests yet; the confidence ladder is read from `figures.json`, never typed)
...
- [**The BUSY Bar surface — witness state on someone else's glass (design)**](design/busybar_surface.md) — the first SecuraCV *display surface* on third-party hardware, and the argument that makes it safe: the bar's two displays map onto the Canary Cards privacy classing, so the room-facing 72x16 matrix renders public phrases only and the operator-facing rear carries the detail. Built and **bench-unvalidated** (the list of what a real device has to settle is §9). The three ideas worth stealing: PII on the front is a `can't` (the renderer takes a closed phrase type, not a string, so `K. AT DOOR` cannot be typed OR received), a dead-man's switch (every drawing expires on the device sooner than the surface promises to redraw it, so a dead bridge goes blank instead of holding a stale calm state), and stillness-as-the-alert (the breath *is* the liveness proof — no flashing alert states, ever). Plus why no control on it can affect witnessing, stated three ways and grepped for in CI
...
- [Timeline history deeper than the ring (design)](design/witness_history_bridge.md) — both stages built, no bench pass yet: the canary's web timeline pages past the 32-record RAM ring into the card through a loop-task SD read bridge — the pure backward JSONL walker (any chunk size, torn-line skip, an untrusted O(page) resume hint, chain linkage) and the bridge's state machine (one request slot, a generation counter, a 3 s bounded wait, at most 4 KiB read per loop pass) are host-tested; `GET /api/witness` serves card pages with no new route, and rows are badged "from card, chain-linked", never "verified"
...
- **Improvement roadmap (2026-09):** [the three-repo audit's open list, ordered](IMPROVEMENT_ROADMAP.md) — 138 findings across the monorepo, the HACS mirror and the website; what landed in PRs #1628 / #7 / #183, the 60 that are still open with a fix and an effort beside each, the device-package proposal (one parametric manifest per product feeding firmware envs, emulator, CAD, glTF, figures, flasher and site), and the Wi-Fi sensing path from compiles to a bench pass
- **TODO sweep backlog (2026-09):** [the cross-session work plan](audit/repo_todo_sweep_2026-09.md) — a marker-level sweep of all three repos (stubs, deferred features, unchecked checklists, placeholder data) distilled into pickable items with stable IDs and gate tags; leads with the seven human unlocks (bench session, key ceremony, Apple account, FCC, Stripe, mirror PAT, brands submission) that un-gate whole sections. Pick an item, do it, tick it in the same PR
