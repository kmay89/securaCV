...
| **Canary Doorbell** | design | *Design-stage* — the doorbell that **cannot** become a surveillance camera: no live view, no inbound talk, no clips, no faces (Invariants I/II), and in their place a signed package custody chain, radar approach/dwell, a sub-second LAN ring and a glow ring that never lies. The released [Vision Doorbell case](hardware/enclosure/canary_vision_doorbell.scad) is its Lite tier today; the Linux-core Pro is paper. See the [doorbell dossier](hardware/canary_doorbell_research.md). |
