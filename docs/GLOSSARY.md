...
| **Canary Doorbell** | design | *Design-stage* — the doorbell that **cannot** become a surveillance camera: no live view, no inbound talk, no clips, no faces (Invariants I/II), and in their place — as design targets, none benchmarked — a signed small-object custody chain on the doormat, radar approach/dwell, a LAN ring targeted at sub-second, and a glow ring that never lies. The released [Vision Doorbell case](hardware/enclosure/canary_vision_doorbell.scad) is a doorbell-shaped case for a Vision today (its button is not yet read by firmware); the Linux-core Pro is paper. See the [doorbell dossier](hardware/canary_doorbell_research.md). |
