...
- **Ed25519 signatures**: Domain-separated via `DOMAIN_*` constants (sealed log, break-glass receipt, export receipt, break-glass token, trustee approval, device-key rotation). Prevents cross-domain signature confusion attacks. (Trustee approvals were previously signed over the bare request hash with no domain separation — the one context that bypassed it; they are now bound to `DOMAIN_TRUSTEE_APPROVAL` so a signature minted for another purpose cannot stand in for a trustee's consent.) Domain separation is now **mandatory** for every context: the pre-domain-separation "bare entry-hash" Ed25519 verify fallback that `SignatureMode::Compat` once accepted has been retired, so an undomained signature no longer verifies in any mode. `Compat` now differs from `Strict` only in that the post-quantum signature is optional.
