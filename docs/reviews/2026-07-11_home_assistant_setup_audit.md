...
| F7 | Medium | `custom_components/securacv/__init__.py` | `ap_ip`/`ip` from an untrusted MQTT status payload was interpolated directly into the device page's clickable `configuration_url` (phishing vector: `http://<attacker-host>`). | New `_safe_config_url()` accepts only the address forms devices actually report: a private/link-local IP (IPv6 bracketed) or a `.local` mDNS hostname — no public hosts, schemes, ports, paths, or credentials. |
