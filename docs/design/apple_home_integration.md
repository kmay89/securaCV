...
| **A2** | P0 | app as shepherd + concierge + Doctor card; per-signal consent | **mostly built** — `HomeKitBridge.swift` is the honest shepherd (vocabulary mirror, real `HMHomeManager` authorization + homes readback, lazy so nothing touches HomeKit before consent), and the UI renders it: `AppleHomeView` (master + per-signal consent, tamper refusal visible), the per-witness standing + Doctor row in `DeviceDetailView`. And the concierge is built: `AutomationConciergeSheet` (signal → your scene → the sentence, written as a real `HMEventTrigger` + action set on the home hub; appears only when accessories are visible, per §6), with the pure half — `PlannedAutomation`, the `ConciergeReadiness` ladder, the characteristic-type map — host-tested in `HomeAutomationAuthor.swift`. Inbound (decision #4) and postures (#3) stay out |
