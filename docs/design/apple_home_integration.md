...
| **A0** | P0 | HA HomeKit Bridge worked recipe ([`home-assistant-homekit-bridge.md`](../integrations/home-assistant-homekit-bridge.md)) | **built** — includes the lane-choice table and states plainly that this path is un-paced (decision #8) |
| **A1** | A0 | native motion/occupancy binary_sensors in `custom_components/securacv` | **built** — driven by `event_signals`, now dictionary-governed and linter-gated across Rust + Python; the recipe's template section is demoted to a pre-0.6.0 appendix |
