...
released devices' assembled depths by 30–58 %. The same file carries the
**seams** (each part's visible band along the depth), which the device
massings draw between, so the drawn stack nests the way the printed parts
do. The generator lives beside the CAD because measuring needs OpenSCAD and
this generator must not; the enclosure CI re-measures on every CAD change,
and a multi-part figure with no measured row is refused, never approximated.
The multi-part drift guard compares the drawn massing to that measured
envelope (height gets ±2.5 mm — a banded massing centers its parts
vertically, and small assembly offsets like the doorbell plate's foot are
beneath its fidelity; the ledger still publishes the exact measured number).
