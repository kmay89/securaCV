...
A **multi-part device** (the assembled WAP, Vision, Sense, Doorbell) gets
its envelope from a second measured source:
`docs/hardware/enclosure/assembled_dims.json`, generated by
`gen_assembled_dims.py`, which renders the parts in their fit-checked
assembled positions and measures the union — because stacking part bounding
boxes front-to-back ignores every nesting lip, and doing so overstated the
released devices' assembled depths by 30–58 %. The positions are the
device's **resting, mounted attitude**, not merely parts-centered: the
doorbell's body and face ride half a millimeter up the mounting slide onto
the plate's L-foot (the case's own datum), and that half-millimeter is in
the published height. The same file carries the **seams** (each part's
visible band along the depth), which the device massings draw between, so
the drawn stack nests the way the printed parts do. The generator lives
beside the CAD because measuring needs OpenSCAD and this generator must
not; the enclosure CI re-measures on every CAD change — the envelope AND
the seams, since an assembly datum can move the bands without moving the
outer box — and a multi-part figure with no measured row is refused, never
approximated. The multi-part drift guard compares the drawn massing to that
measured envelope (height gets ±2.5 mm — a banded massing centers its
