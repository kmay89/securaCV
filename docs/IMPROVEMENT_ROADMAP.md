...
| Landed in wave 8 (PR #1691, website #199) | the open clauses of rows 12 (both flashers' receipts name the broker TLS mode they sealed) and 17 (the iPhone's Display settings sheet reads the glass's `on_glass` block, read-only), and three follow-ups no row listed, from a fresh security sweep of the wave-5 and wave-6 surfaces: the Canary API's credential carry, `Host` guard and CA verdict (row 12's canary client), the security docs rewritten to the shipped posture with a banned-claims test (one passage older than #1704's page-token gate survived; sweep D8), and the size-guard rule for every product (wave 6's row-12 prelude, made structural). The PR's own title says wave 7; the row above already holds that name, so this document calls it wave 8. Website #199 makes the Wall read a silent `online` as offline. See §3, "Landed in wave 8" |
...
ledger edits, so these lines were written after the merge.
...
- **Row 12 (the canary client) — the Canary's API keeps its broker password
  ...
  `credential_carry`). Every gated route answers a `Host` that does not name
  the device with 403, and the page carries no token for it, against DNS
  rebinding; a request over the Canary's own setup AP is exempt by
  interface (`firmware/common/network/host_guard.h`, moved from the
  display). A CA the transport reads back empty is `409 ca_unreadable`, not
  Ok. Recorded in `firmware/canary/CONSOLIDATION.md` ("Later additions to
...
- **The security docs say what ships, with a gate.** `SECURITY_MODEL.md`
  ...
  model still says the flagship's page hands its token to whoever loads it;
  sweep D8), and two CSI sentences it carried (`FEATURES.md`'s probe row and
...
- **Website #199 — the Wall reads a silent `online` as offline.**
  ...
  by sha256 (sweep W21 moves it into the carry). The download page offers
  the Flasher, and stale status copy is corrected across the site (sweep
  W20). The apps' vendored copies of the emulator still default a silent
  row to present (sweep A24).
