...
and CI3 packages with four more (F53, F55, HA14 and CI2); and PR #1727
...
- **The key-pinning steps say where each product's key is (PR #1725, sweep
  ...
  in PR #1727, below) and canary-display has no key, and warn that a
  canary-wap may read as a mismatch today (HA18; PR #1727 fixes it in Home
...
- **Home Assistant verifies a canary-wap's signed publishes (PR #1727, sweep
...
- **Canary Sense and Sentinel print their full key at boot (PR #1727, sweep
...
- **Two firmware fixes HA18 turned up (PR #1727, sweeps HA19 and HA21).**
...
- **The display's join hints fit narrow glass, and the WAP publishes in
  lowercase (PR #1727, sweeps F50 and HA20).** A failed join's fix and the
...
- **Row 17's last open clause: the Quiet Hours wheels center on every touch
  glass (PR #1727).** `mk_hour_roller` took the Location wheels' center
