...
| Landed after wave 3 (Apple-toolchain rows, compile-untested there) | 3 |
| Still open | 4 in full, 2 in part |
...
### Landed after wave 3: the Apple-toolchain rows

The three rows that share `ios/Sources/SecuraCV/Transport/DeviceAPI.swift`
(5, 6, 13) landed together on 2026-09-08 in a worktree with no Swift
toolchain: every Swift line is read-carefully and **compile-untested here** —
the gated iOS CI is its compiler — while the canary-wap endpoint, the shared
fixture and its generator are host-tested. Each row below says what shipped
and what it could not prove. What is left — 4 rows in full and 2 in part — is
hardware-bound (2, 22, the §5 bench steps), a maintainer decision (30, 42,
51), and the device package's Parametrize wave.
