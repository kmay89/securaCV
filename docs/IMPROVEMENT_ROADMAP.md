...
- **The Canary's NVS writes say what they wrote (PR #<D>, sweep F55).**
  ...
  bench-tested. The identity-key store (F58) and canary-wap's copies (F59)
  are open.
...
- **Row 37 and the workflows: one host-test list, and a read gate on
  ...
  `scripts/check_path_filter_reads.py` fails a PR whose tests read a file
  outside the filter; its first run added 29 paths. Host-tested locally;
  PR #<D>'s CI is their first run on GitHub. The drift-step generators and
  the other filtered workflows are not recorded yet (CI4).
