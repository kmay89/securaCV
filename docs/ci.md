...
|---|---|---|
| **Lint gates** | [`lint.yml`](../.github/workflows/lint.yml) | The repo-wide scripted checks (the workflow file is the full list): docs-map reachability (`scripts/lint_docs_index.py`), markdown link/anchor integrity across every tracked `.md` (`scripts/lint_md_links.py`), US spelling (`scripts/lint_spelling.py`), the fleet-not-bird-word rule (`scripts/lint_fleet_word.py`), dictionary sync (`scripts/lint_dictionary_sync.py`), agent-entrypoint drift (`scripts/gen_agent_entrypoints.py --check`), Apple Home doc drift (`scripts/gen_apple_home_docs.py --check`), no-impersonation, feature-flag hygiene, build-matrix sync, app-version sync, BOM schema, manifest-owned CAD knobs (`docs/hardware/enclosure/gen_cad_params.py --check`), the derived display env list (`firmware/scripts/flavor_envs.py --check-workflows`), the firmware SBOM (`scripts/gen_firmware_sbom.py --check --validate`), no committed benchmark number (`scripts/lint_bench_rows.py`), the `scripts/tests` suites (among them the security-docs claims gate and the hub plan's prose gate), and more. It is unfiltered, so a check whose input is prose anywhere in the tree runs here. The most-tripped ones are tabled in [`AGENTS.md`](../AGENTS.md) ("CI gates you will trip"). |
...
| **Lab / emulator drift** | `canary-local.yml`, `emulator-dist-refresh.yml` | Firmware → WASM boots in a browser; the "Dist drift check" byte-compares the committed `canary-local/emulator/dist/` bundles, and the page-logic tests catch a stale `fw_version` stamp; the page-logic job also fails when a node or python3 test reads a file outside the workflow's path filter (`scripts/check_path_filter_reads.py`; python3 existence checks and shell reads are not recorded). The full stale-dist playbook is in [`CLAUDE.md`](../CLAUDE.md) ("Generated files"). |
