...
| **FR-14** | **Nothing ships unvetted or unverifiable.** Deps pass cargo-deny (+vet for Class A); releases carry signed provenance with committed SBOMs; installers verify checksums; every node reports version+config hash at startup; one version, one gate. | Version-lockstep lint (have, since #909); cargo-audit + dependabot (have); cargo-deny — licenses/bans/sources per `deny.toml` in audit.yml (have); SLSA provenance on firmware artifacts (have — `docs/supply_chain_transparency.md`); cargo-vet, cosign, pinned installer (open) | 🟡 |
