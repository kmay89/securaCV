...
> boundary events are still signed/logged but **no frame is sealed into the vault**, with the exact
> steps to enable it (`src/bin/witnessd.rs`). This mirrors the F-01 fix (no silent, undocumented
> default). The doc-debt half was already done (`v1-roadmap.md` describes the vault as "wired,
