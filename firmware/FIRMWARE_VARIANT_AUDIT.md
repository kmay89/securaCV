...
**Last updated:** 2026-08-21 (added canary-sentinel / canary-companion / canary-fence-guard rows; reworded canary-ota to reflect the 2026-06 engine promotion to `firmware/common/ota/`). Previous: 2026-08-20 (refreshed canary-vision + canary-sense onboarding readiness: both now raise the shared setup portal from `firmware/common/network/setup_portal` when unprovisioned or recovery-stuck, alongside flasher NVS credential seeding)
...
| `firmware/projects/canary-sentinel` | SPECIALIZED (Phase 0) | Doorway/window multi-sensor fusion guardian: five presets composing the host-tested `common/fusion` brain | Fusion core host-tested in CI; build envs compile-checked; not in `flavors.json`; hardware bench pending | Low (console transitions only in Phase 0) | Low (Phase 1 adds the sense-style net/witness stack) | Medium until Phase 1 lands |
