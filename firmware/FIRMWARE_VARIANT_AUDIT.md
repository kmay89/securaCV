...
**Last updated:** 2026-09-23 (canary-sentinel row: Phase 1a network/witness path compile-gated in CI, unreleased). 2026-08-21 (added canary-sentinel / canary-companion / canary-fence-guard rows; reworded canary-ota to reflect the 2026-06 engine promotion to `firmware/common/ota/`). Previous: 2026-08-20 (refreshed canary-vision + canary-sense onboarding readiness: both now raise the shared setup portal from `firmware/common/network/setup_portal` when unprovisioned or recovery-stuck, alongside flasher NVS credential seeding)
...
| `firmware/projects/canary-ota` | SPECIALIZED | Standalone ESP-IDF OTA test/demo harness (the engine it pioneered was promoted to `firmware/common/ota/` in 2026-06, which the ACTIVE variants consume) | Engine promotion 2026-06; harness built by no CI workflow since | N/A (not a WAP UX product) | N/A | Medium (outside CI; engine successor is CI-covered) |
| `firmware/projects/canary-display` | SPECIALIZED | Fleet status displays (watch puck + 4.3" dash): MQTT subscribe, TOFU + Ed25519 chain verify, glance UI | Active — 2.4.x releases through 2026-08 (watch, dash, nightstand, touch169, AMOLED flavors; CI-verified, AMOLED first light bench-verified 2026-08-23) | **High** (the product IS an on-device UI) | Medium/High (SoftAP + captive-portal first boot, join QR on the glass — `src/net/provision.cpp`; compiled secrets remain an option for kiosk builds) | Medium until every flavor is bench-validated |
| `firmware/projects/canary-sentinel` | SPECIALIZED (Phase 1a) | Doorway/window multi-sensor fusion guardian: five presets composing the host-tested `common/fusion` brain | Fusion core host-tested in CI; Phase 1a network/witness path compile-gated in CI (`door` + `lite` in `flavors.json`, `unreleased`); onboard-radio channels (Phase 1b) not built; hardware bench pending | Low (headless: USB console + HA entities from MQTT discovery) | Medium (canary-sense's shared setup portal + NVS provisioning — not yet run on hardware) | Low (compile-gated in CI; its net stack is pinned to canary-sense's by `check_sentinel_net_sync.sh`) — Medium until the bench checklist is green |
| `firmware/projects/canary-companion` | SPECIALIZED (Phase 0) | Night Watch (truly-dark AMOLED bedside clock) + Pocket Canary (virtual pet) on the AMOLED-2.06 wrist board | Pure cores host-tested in CI; no PlatformIO env or runtime yet | N/A (no runtime yet) | N/A | Low (honestly labeled Phase 0) |
| `firmware/projects/canary-fence-guard` | CONCEPT (pre-Phase-0) | Perimeter fence witness over Meshtastic/LoRa — research staged, firmware pending | `src/main_stub.cpp` `#error`s on purpose; nothing builds or ships | N/A | N/A | N/A (deliberately unbuildable) |
