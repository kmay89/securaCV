...
| `csi_wander_jitter` | A second extractor, **off by default**. A build with `-DCSI_WANDER_JITTER=1` writes amplitude-centroid wander to `v[28]` and mean frame-to-frame jitter to `v[29]`, both computed within one window from the same AGC-normalized rows; without the flag both slots stay zero, and `v[30..31]` are reserved either way. Header-only; also home to the library's one `isqrt_u32`. Host-tested on synthetic frames; flag off in every shipped build; no bench numbers, no thresholds, not read by any module. See [`docs/csi_modules.md`](../../../docs/csi_modules.md#the-second-extractor-wander-and-jitter). | `csi_wander_jitter.h` |
