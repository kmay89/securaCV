...
|---|---|---|:---:|
| **`chirp_channel.h`** | `firmware/common/chirp/chirp_channel.h` | **The canonical C-ABI chirp-channel API** (Community Witness Network: ephemeral identity, structured templates, ≤3-hop relay, cooldowns). Consumed by canary-wap (PIO) `src/main.cpp`. | ✅ |
...
| ESP-NOW recv callback | register chirp's dispatcher on `mesh_transport::set_recv_callback` in `main.cpp` under `#if FEATURE_CHIRP` | mirrors how `mesh_session` receives. |
...
| `health_log()` + `SCV_LOG_*` / `SCV_CAT_*` (`health_log.h`) | `health_log()` in `firmware/common/health/health_log.h`; `SCV_LOG_*`→`HEALTH_LOG_*`, add `HEALTH_CAT_CHIRP` | integration hook wired in `main.cpp`. |
