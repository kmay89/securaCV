...
| `xiao-esp32c6-sentinel` | ESP32-C6 | compile-tested | canary-sentinel (Phase 1a, unreleased) | Canary Sentinel Standard/Heavy head — MR60BHA2 radar + PIR + lux + WiFi/BLE; fusion core host-tested, network/witness build compile-gated in CI (env `canary-sentinel-door`), bench pending |
| `xiao-esp32c3-sentinel-lite` | ESP32-C3 | compile-tested | canary-sentinel (Phase 1a, unreleased) | Canary Sentinel Lite — PIR + lux + WiFi/BLE, no radar (honest tier limit); fusion core host-tested, network/witness build compile-gated in CI (env `canary-sentinel-lite`), bench pending |
