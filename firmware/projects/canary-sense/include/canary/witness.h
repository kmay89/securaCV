...
//
// Chain: a domain-separated SHA-256 hash chain over the signed event
// canonicals (domains from common/witness/witness_chain.h):
//   payload_hash = SHA256("securacv:fw:payload:v1" || 0x00 || canonical)
//   head'        = SHA256("securacv:fw:chain:v1"   || 0x00 || head || payload_hash)
// The genesis head binds the chain to the device key:
//   head0 = SHA256("securacv:fw:boot:v1" || 0x00 || pubkey)
