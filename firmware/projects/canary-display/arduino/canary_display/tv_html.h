...
<!-- tv/index.html — "Canary TV": a 10-foot ambient security surface for any
     ...
     Query options (all optional):
       ...
       ?demo=1                    render a synthetic fleet, loudly labelled DEMO
...
/* Transport / freshness banner — AD-Core: a dead transport is bannered
   within one render cycle; last-known is labelled as such. */
#banner{
...
/* ── Roll-call: every witness, always with label + glyph, never colour
...
(function(){
  ...
  // Never colour-only: each state carries word + glyph + border position.
  var SEV = [
  ...
  // ── demo fleet (loudly labelled; can never be mistaken for a real one,
