...
// main.cpp to shed load (stretched sensor cadence) under memory pressure.
...
enum class Level : uint8_t {
  ...
  Critical,    // free < HEAP_CRITICAL_BYTES — shed load (2x sensor period)
  Emergency,   // free < HEAP_EMERGENCY_BYTES — survival mode (5x period)
};
...
// Multiplier main.cpp applies to its sensor cadence at the current level:
