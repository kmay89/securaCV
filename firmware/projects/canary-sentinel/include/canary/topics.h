...
// canary-sentinel's MQTT topics — the canary-sense schema minus the radar
// reflex dials (cfg/*) and the identify button, which Phase 1a does not wire.
struct Topics {
  char events[96];
  char state[96];
  char status[96];
  // Witness trust surface — same wire schema as canary-wap / canary-sense:
  // HA TOFU-pins the pubkey from the retained health payload and verifies the
  // signed chain head with its existing verify_chain path.
  char chain[96];
  char health[96];
