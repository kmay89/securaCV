 ...
 * @brief csi_event module for the WAP's own integrity story (system.integrity).
 *
 ...
 * detect. The WAP has no accelerometer, no enclosure switch, no tamper
 * GPIO wired in any profile, and no GPS on the base build — so `motion`,
 * `enclosure`, `gpio`, and `gps_jamming` are never emitted here, and
 * advertising a tamper sensor that cannot fire would be a false promise of
 * protection. What it CAN say honestly:
 *
 ...
 *
 * The module owns every transition rule; the .ino's loop feeds it the three
 * facts only the sketch can see together (reset classification + SD state)
