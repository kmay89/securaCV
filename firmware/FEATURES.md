...
| SoftAP WPA2/WPA3 transition + PMF capable (F16; WPA2 where the core lacks SoftAP SAE, reported as `ap_auth`; compile-tested, no bench) | ⚠️ | ⚠️ | ❌ | ❌ | ❌ | ➖ | ❌ |
...
| TLS (HTTPS) support | ✅ Self-signed cert | ❌ | ⚠️ ECDSA P-256 self-signed, `FEATURE_HTTPS` in dev/full (compile-tested, release pending slot budget, no bench) | ❌ |
