...
| `GET /api/witness` | ✅ | ❌ | ⚠️ the timeline's pages: the 32-record RAM ring, then — past its oldest record — pages from the SD card, read by the loop task through the history bridge (F35: `?before=`/`?hint=`, rows badged chain-linked, never verified; state machine host-tested, CI-compiled, no bench) | ❌ |
