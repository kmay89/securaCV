...
// SECURACV_REQUIRE_FLASH_ENCRYPTION=1 and flash encryption is not active —
// the decision lives in common/identity/key_at_rest.h. Default builds never
// refuse (Tier 0 of docs/design/hardware_root_of_trust.md is the accepted
// default) and report the posture instead.
bool nvs_load_key(uint8_t priv[32]);
bool nvs_store_key(const uint8_t priv[32]);
// The wire label for where the identity key sleeps right now, as read from
// the eFuses: "plaintext-nvs" | "flash-encrypted" | "flash-encrypted+secure-boot"
// (key_at_rest::wire_label). Carried by /api/status, the health export, the
// 'f' console card and the self-manifest as `key_at_rest`. Not secret material.
