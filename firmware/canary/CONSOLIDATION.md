...
| 16 | Committed CSI events over MQTT (`events` topic, signed, + `tamper` bridge) | ⚠️ live publish + Ed25519 signing (`src/csi_event_egress.cpp`, the shared `common/csi/src/csi_event_wire.h` body; CI compile only, bench pending); no SD event log or reconnect backfill yet | `canary_wap/csi_event_log.{h,cpp}` (SD.h-bound; needs an adapter honoring this tree's loop-task single-writer rule) | Medium | Low |
