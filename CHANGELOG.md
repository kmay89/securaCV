...
- **DB key decoupled from the signing key** (F-04 / Stream B2 prerequisite): set
  ...
  (`resolve_db_encryption_key`) instead of the Ed25519 signing key, so the device identity can be
  rotated without re-encrypting the database. `rekey_database_file()` rotates the DB key itself in
  place (`PRAGMA rekey`). Backward compatible — without the env var, the legacy signing-key
  derivation is byte-identical, so existing databases open unchanged. See
